/*
  10_FactoryTest - ชุดทดสอบโรงงานสำหรับบอร์ด Massmore BME280 (SKU-1023)

  รันเองทันทีหลังบูต ไม่ต้องพิมพ์อะไร  พิมพ์ r เพื่อทดสอบซ้ำ

  สิ่งที่ทดสอบ
    ด่านที่ 1  สแกนบัส I2C หา 0x76 หรือ 0x77
    ด่านที่ 2  ตรวจรหัสประจำรุ่นว่าเป็น Bosch BME280 ของแท้ (0x60)
    ...
    RUN TEST   ทดสอบทุกความสามารถของชิปอีก 22 หัวข้อ
...
static const char *ftF2(char *buffer, size_t size, float value) {
  ...
  snprintf(buffer, size, "%s%ld.%02ld", negative ? "-" : "", scaled / 100, scaled % 100);
  return buffer;
}
...
static const char *ftF3(char *buffer, size_t size, float value) {
  ...
  snprintf(buffer, size, "%s%ld.%03ld", negative ? "-" : "", scaled / 1000, scaled % 1000);
  return buffer;
}
...
static void ftRecord(const char *name, FtStatus status, const char *detail) {
  ...
  char line[416];
  snprintf(line, sizeof(line), "%s %02u %-14s %s", tag, (unsigned)(g_resultCount + 1), name,
           result.detail);
  Serial.println(line);

  snprintf(line, sizeof(line), "#RESULT,%u,%s,%s,%s", (unsigned)(g_resultCount + 1), name,
           word, result.detail);
  Serial.println(line);
...
static bool gateBusScan() {
  ...
  if (deviceCount == 0) {
    ftCopyDetail(detail, sizeof(detail),
                 "ไม่พบอุปกรณ์บนบัสเลย ตรวจ VIN GND SDA SCL และสาย Qwiic");
  ...
  if (g_foundAddress == 0) {
    snprintf(detail, sizeof(detail), "พบ %u อุปกรณ์ แต่ไม่มีตัวไหนอยู่ที่ 0x76 หรือ 0x77",
             (unsigned)deviceCount);
    ftRecord("I2C_SCAN", FT_FAIL, detail);
    return false;
  }

  snprintf(detail, sizeof(detail), "พบเซ็นเซอร์ที่ 0x%02X (อุปกรณ์บนบัสทั้งหมด %u ตัว)",
...
static bool gateChipId() {
  ...
  if (Wire.endTransmission(false) != 0 ||
    ...
    ftCopyDetail(detail, sizeof(detail), "มีอุปกรณ์ตอบที่ address แต่อ่านรีจิสเตอร์ 0xD0 ไม่ได้");
  ...
  if (g_chipId != MASSMORE_BME280_CHIP_ID_BME280) {
    snprintf(detail, sizeof(detail), "รหัส 0x%02X = %s ไม่ใช่ BME280 (ต้องเป็น 0x60)", g_chipId,
             MassmoreBME280::chipToString(g_chipType));
    ftRecord("CHIP_ID", FT_FAIL, detail);
    return false;
  }

  snprintf(detail, sizeof(detail), "รหัส 0x60 = Bosch BME280 ถูกต้อง");
  ftRecord("CHIP_ID", FT_PASS, detail);

  if (!bme.begin(g_foundAddress, &Wire)) {
    snprintf(detail, sizeof(detail), "begin() ไม่ผ่าน : %s",
             MassmoreBME280::errorToString(bme.lastError()));
    ftRecord("BEGIN", FT_FAIL, detail);
    return false;
  }
  ftCopyDetail(detail, sizeof(detail), "เริ่มต้นไลบรารีและอ่านค่าชดเชยได้ครบ");
...
static void testSoftReset() {
  ...
  if (ctrlMeas == 0x00 && config == 0x00 && ctrlHum == 0x00) {
    ftCopyDetail(detail, sizeof(detail), "รีเซ็ตแล้วรีจิสเตอร์ตั้งค่าทั้งสามกลับเป็น 0x00");
    ftRecord("SOFT_RESET", FT_PASS, detail);
  } else {
    snprintf(detail, sizeof(detail), "หลังรีเซ็ตได้ ctrl_meas=0x%02X config=0x%02X ctrl_hum=0x%02X",
...
static void testCalibration() {
  ...
  snprintf(detail, sizeof(detail), "T1=%u T2=%d P1=%u P2=%d H1=%u H2=%d", (unsigned)c.dig_T1,
  ...
  if (inRange) {
    ftCopyDetail(detail, sizeof(detail), "สัมประสิทธิ์ทุกตัวอยู่ในช่วงที่ Bosch ใช้จริง");
    ftRecord("CALIB_RANGE", FT_PASS, detail);
  } else {
    ftCopyDetail(detail, sizeof(detail), "สัมประสิทธิ์บางตัวอยู่นอกช่วงปกติ น่าสงสัยว่าไม่ใช่ของแท้");
  ...
  snprintf(detail, sizeof(detail), "H1=%u H2=%d H3=%u H4=%d H5=%d H6=%d", (unsigned)c.dig_H1,
...
static void testRegisterEcho() {
  ...
  if (allOk) {
    ftCopyDetail(detail, sizeof(detail), "เขียน config สามค่าแล้วอ่านกลับได้ตรงทุกบิต");
    ftRecord("REG_ECHO", FT_PASS, detail);
  } else {
    snprintf(detail, sizeof(detail), "เขียน 0x%02X แต่อ่านกลับได้ 0x%02X", badExpected, badValue);
...
static void testStatusRegister() {
  ...
  if (!bme.readStatus(status)) {
    ftCopyDetail(detail, sizeof(detail), "อ่านรีจิสเตอร์ status ไม่ได้");
  ...
  if (reserved == 0) {
    snprintf(detail, sizeof(detail), "status=0x%02X บิตสงวนเป็น 0 ถูกต้อง", status);
    ftRecord("STATUS_REG", FT_PASS, detail);
  } else {
    snprintf(detail, sizeof(detail), "status=0x%02X มีบิตสงวนไม่เป็นศูนย์ (0x%02X)", status,
             reserved);
...
static void testSleepMode() {
  ...
  if (inSleep && notMeasuring) {
    snprintf(detail, sizeof(detail), "ctrl_meas=0x%02X อยู่ในโหมด sleep และไม่มีการวัด", ctrlMeas);
    ftRecord("SLEEP_MODE", FT_PASS, detail);
  } else {
    snprintf(detail, sizeof(detail), "ctrl_meas=0x%02X measuring=%d", ctrlMeas,
             notMeasuring ? 0 : 1);
...
static void testForcedMode() {
  ...
  if (ok && backToSleep) {
    snprintf(detail, sizeof(detail), "วัดเสร็จใน %lu ms แล้วกลับไป sleep เองถูกต้อง",
             (unsigned long)elapsed);
    ftRecord("FORCED_MODE", FT_PASS, detail);
  } else if (ok) {
    snprintf(detail, sizeof(detail), "วัดเสร็จใน %lu ms แต่ ctrl_meas=0x%02X ไม่กลับไป sleep",
             (unsigned long)elapsed, ctrlMeas);
    ftRecord("FORCED_MODE", FT_WARN, detail);
  } else {
    snprintf(detail, sizeof(detail), "สั่งวัดไม่สำเร็จ : %s",
             MassmoreBME280::errorToString(bme.lastError()));
...
static void testNormalMode() {
  ...
  if ((ctrlMeas & 0x03) == 0x03 && measuringSeen > 0) {
    snprintf(detail, sizeof(detail), "วัดวนต่อเนื่อง เจอบิต measuring %u ครั้งจาก 40",
             (unsigned)measuringSeen);
    ftRecord("NORMAL_MODE", FT_PASS, detail);
  } else if ((ctrlMeas & 0x03) == 0x03) {
    ftCopyDetail(detail, sizeof(detail),
                 "อยู่ในโหมด normal แต่จับบิต measuring ไม่ทัน (รอบวัดสั้นมาก ไม่ผิดปกติ)");
    ftRecord("NORMAL_MODE", FT_WARN, detail);
  } else {
    snprintf(detail, sizeof(detail), "ctrl_meas=0x%02X ไม่ได้อยู่ในโหมด normal", ctrlMeas);
...
static void testHumidityChannel() {
  ...
  if (okOff && okOn && rawOff.adc_H == MASSMORE_BME280_RAW_SKIPPED_16BIT &&
    ...
    snprintf(detail, sizeof(detail), "ปิดได้ 0x%04lX เปิดได้ %ld ตอบสนองตาม osrs_h ถูกต้อง",
...
static void testOversamplingTiming() {
  ...
  snprintf(detail, sizeof(detail), "x1 ใช้ %lu ms (สเปก %u) / x16 ใช้ %lu ms (สเปก %u)",
...
static void testNoise() {
  ...
  if (!ftCollect(20, &meanT, &sdT, &meanP, &sdP, &meanH, &sdH)) {
    ftCopyDetail(detail, sizeof(detail), "เก็บตัวอย่างไม่พอ");
    ftRecord("NOISE", FT_FAIL, detail);
    return;
  }

  snprintf(detail, sizeof(detail), "sd อุณหภูมิ %s C  ความดัน %s hPa  ความชื้น %s %%RH",
...
static void testFilter() {
  ...
  snprintf(detail, sizeof(detail), "sd ความดัน ปิดฟิลเตอร์ %s hPa -> ฟิลเตอร์ 16 %s hPa",
...
static void testPlausibility() {
  ...
  massmore_bme280_reading_t r;
  if (!bme.read(r)) {
    snprintf(detail, sizeof(detail), "อ่านค่าไม่สำเร็จ : %s",
             MassmoreBME280::errorToString(bme.lastError()));
    ftRecord("READ_VALUES", FT_FAIL, detail);
    return;
  }

  snprintf(detail, sizeof(detail), "%s C  %s %%RH  %s hPa", ftF2(bufT, sizeof(bufT), r.temperature),
  ...
  snprintf(detail, sizeof(detail), "%s C (ช่วงที่รับได้ %d ถึง %d)",
  ...
  snprintf(detail, sizeof(detail), "%s %%RH (ช่วงที่รับได้ %d ถึง %d)",
  ...
  snprintf(detail, sizeof(detail), "%s hPa (ช่วงที่รับได้ %d ถึง %d)",
  ...
  snprintf(detail, sizeof(detail), "จุดน้ำค้าง %s C ต้องไม่เกินอุณหภูมิ %s C",
  ...
  snprintf(detail, sizeof(detail), "%s m (คำนวณจากระดับน้ำทะเลมาตรฐาน 1013.25 hPa)",
...
static void testStability() {
  ...
  if (!bme.read(first)) {
    ftCopyDetail(detail, sizeof(detail), "อ่านค่าแรกไม่สำเร็จ");
  ...
  if (!bme.read(second)) {
    ftCopyDetail(detail, sizeof(detail), "อ่านค่าที่สองไม่สำเร็จ");
    ftRecord("STABILITY", FT_FAIL, detail);
    return;
  }
  ...
  snprintf(detail, sizeof(detail), "ใน 1.5 วินาที อุณหภูมิเปลี่ยน %s C ความชื้นเปลี่ยน %s %%RH",
...
static void testBusSpeed() {
  ...
  if (readOk && id == MASSMORE_BME280_CHIP_ID_BME280 && measureOk) {
    ftCopyDetail(detail, sizeof(detail), "ทำงานได้ปกติที่ความเร็วบัส 400 kHz");
    ftRecord("BUS_400K", FT_PASS, detail);
  } else {
    ftCopyDetail(detail, sizeof(detail),
                 "ที่ 400 kHz สื่อสารไม่ผ่าน ให้ใช้ 100 kHz หรือเปลี่ยนสาย Qwiic ที่สั้นลง");
...
static void testGenuine() {
  ...
  snprintf(detail, sizeof(detail), "ผ่าน %u จาก 10 ข้อ (id=%d calib=%d%d%d%d reset=%d hum=%d echo=%d meas=%d live=%d)",
...
static void testPresets() {
  ...
  if (ok) {
    ftCopyDetail(detail, sizeof(detail), "ชุดตั้งค่าสำเร็จรูปทั้งสี่ชุดเขียนลงชิปได้ปกติ");
    ftRecord("PRESETS", FT_PASS, detail);
  } else {
    ftCopyDetail(detail, sizeof(detail), "ตั้งชุดสำเร็จรูปไม่ผ่าน");
...
static void printSummary(bool gatesPassed) {
  ...
  for (uint8_t i = 0; i < g_resultCount; i++) {
    const char *tag = (g_results[i].status == FT_PASS) ? "[ OK ]"
                      : (g_results[i].status == FT_WARN) ? "[WARN]"
                                                         : "[FAIL]";
    char line[416];
    snprintf(line, sizeof(line), "  %s %02u %-14s %s", tag, (unsigned)(i + 1),
  ...
  char line[416];
  snprintf(line, sizeof(line), "#DEVICE,0x%02X,%s,0x%02X,%s,%u", g_foundAddress,
...
static void runFactoryTest() {
  ...
  if (!gateBusScan()) {
    ...
    Serial.println(F("    5. ถ้าบัดกรีจัมเปอร์ ADDR ไว้ ต้องหาที่ 0x77"));
    printSummary(false);
    return;
  }
